...
const STATUS_META: Record<string, { icon: React.ReactNode; bg: string; text: string; dot: string }> = {
  ...
  served:       { icon: <CheckCircle2 className="h-3 w-3" />,   bg: 'bg-[#d1fae5]', text: 'text-[#065f46]', dot: 'bg-[#10b981]' },
  completed:    { icon: <CheckCircle2 className="h-3 w-3" />,   bg: 'bg-[#d1fae5]', text: 'text-[#065f46]', dot: 'bg-[#10b981]' },
    ...
    return (
      <div className="bg-white rounded-xl border border-[#e6e6e7] shadow-sm overflow-hidden">
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#e6e6e7] bg-[#fafafa]">
              {['Order #', 'Customer', 'Items', 'Total', 'Status', 'Payment', 'Time', 'Actions'].map((h) => (
                <th key={h} className="px-3 py-2.5 text-left text-[9px] font-semibold text-[#757575] uppercase tracking-wider">{h}</th>
              ))}
            </tr>
          </thead>
                ...
                <React.Fragment key={row.orderNumber}>
                  <tr
                    className={cn(
                      ...
                      isExpanded ? 'bg-[#f0f9f4]' : 'hover:bg-[#f8faf9]',
                  ...
                  >
                    ...
                    <td className="px-3 py-3">
                      <div>
                        <p className="text-[12px] font-bold text-[#111814]">{row.orderNumber}</p>
                        <p className="text-[10px] text-[#94a399]">{row.type}{row.table !== '—' ? ` · ${row.table}` : ''}</p>
                      </div>
                    </td>
                    {/* Customer */}
                    <td className="px-3 py-3">
                      <div>
                        <p className="text-[11px] font-medium text-[#111814]">{row.customer}</p>
                        {row.phone && <p className="text-[10px] text-[#94a399]">{row.phone}</p>}
                      </div>
                    </td>
                    {/* Items */}
                    <td className="px-3 py-3">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium bg-[#f1f5f2] text-[#47554d] rounded-full">
                    ...
                    {/* Total */}
                    <td className="px-3 py-3">
                      <span className="text-[12px] font-bold text-[#111814] tabular-nums">
                        {currSymbol}{row.total.toLocaleString('en-IN')}
                      </span>
                    </td>
                    {/* Status */}
                    <td className="px-3 py-3">
                    ...
                    <td className="px-3 py-3">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {row.paymentStatus === 'paid' && row.walletAmountUsed > 0 && (
                          <span className="inline-flex items-center gap-1 text-[9px] text-[#0f7b47] font-medium">
                            <Wallet className="h-2.5 w-2.5" />
                            {row.paymentMethod === 'Wallet' ? 'Wallet (full)' : `Wallet ${currSymbol}${row.walletAmountUsed.toLocaleString('en-IN')}`}
                          </span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <div>
                        <p className="text-[11px] text-[#47554d]">{row.time}</p>
                        <p className="text-[10px] text-[#94a399]">{fmtShortDate(row._raw.createdAt)}</p>
                      </div>
                    </td>
                    {/* Actions */}
                    <td className="px-3 py-3" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-1.5 text-[#94a399] hover:text-[#0f7b47] hover:bg-[#f0f9f4] rounded transition-colors"
                          title="View details"
                        >
                        ...
                        {canEdit && (
                          <button
                            ...
                            className="p-1.5 text-[#94a399] hover:text-[#3b82f6] hover:bg-blue-50 rounded transition-colors"
                            title="Quick edit"
                          >
                        ...
                        {canCancel && (
                          <button
                            ...
                            className="p-1.5 text-[#94a399] hover:text-red-500 hover:bg-red-50 rounded transition-colors"
                            title="Cancel order"
                          >
                        ...
                        {row.phone && (
                          <button
                            ...
                            className="p-1.5 text-[#94a399] hover:text-green-500 hover:bg-green-50 rounded transition-colors"
                            title="Send WhatsApp"
                          >
                  ...
                  {isExpanded && (
                    <tr className="bg-[#f0f9f4] border-b border-[#e6e6e7]">
                      <td colSpan={8} className="px-4 py-3">
                        <div className="grid grid-cols-4 gap-4 text-[11px]">
                          <div>
                            <span className="text-[#94a399] text-[10px] block">Server</span>
                            <span className="font-medium">{row.server}</span>
                          </div>
                          <div>
                            <span className="text-[#94a399] text-[10px] block">Payment Method</span>
                            <span className="font-medium">{row.paymentMethod}</span>
                          </div>
                          <div>
                            <span className="text-[#94a399] text-[10px] block">Order Type</span>
                            <span className="font-medium">{row.type}</span>
                          </div>
                          <div>
                            <span className="text-[#94a399] text-[10px] block">Created</span>
                            <span className="font-medium">{fmtDate(row._raw.createdAt)}</span>
                          </div>
                          {row._raw.cancelReason && (
                            <div className="col-span-4">
                              <span className="text-[#94a399] text-[10px] block">Cancel Reason</span>
                              <span className="font-medium text-red-600">{row._raw.cancelReason}</span>
                            </div>
                          )}
                          {row._raw.notes && (
                            <div className="col-span-4">
                              <span className="text-[#94a399] text-[10px] block">Notes</span>
                              <span className="text-[#47554d]">{row._raw.notes}</span>
                            </div>
                          )}
                        </div>
                      </td>
        ...
        {filtered.length === 0 && !isLoading && (
          <div className="flex flex-col items-center justify-center h-32 text-[#94a399] text-xs gap-1">
    ...
    <div className="flex h-screen w-screen overflow-hidden bg-[#f8faf9] select-none font-sans">
      ...
      <div className="flex flex-1 flex-col min-w-0 ml-[56px]">
        ...
        <header className="flex items-center h-[48px] px-4 bg-white border-b border-[#e2e8e4] shrink-0">
          <h1 className="text-[14px] font-bold text-[#111814]">Nexora Solution</h1>
          <span className="h-4 w-px bg-[#dee2e6] mx-2.5" />
          <span className="text-[11px] text-[#94a399] font-medium">Orders</span>
          <div className="flex-1" />
          <div className="flex items-center gap-2">
            <button
              onClick={handleRefresh}
              disabled={isLoading}
              className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-medium text-[#47554d] border border-[#dee2e6] rounded hover:bg-[#f1f5f2] transition-colors disabled:opacity-50"
            >
              <RefreshCw className={cn('h-3 w-3', isLoading && 'animate-spin')} />
              Refresh
            </button>
            <button onClick={() => setShowReport(true)} className="flex items-center gap-1 px-2.5 py-1 text-[10px] font-medium text-[#0f7b47] border border-[#0f7b47]/30 rounded hover:bg-[#f0f9f4] transition-colors">
        ...
        <div className="flex items-center gap-4 px-4 h-[50px] bg-white border-b border-[#dee2e6] shrink-0">
          {[
            { l: `Total (${statsLabel})`, v: stats.total },
            { l: 'Active', v: stats.active },
            { l: 'Revenue', v: `${currSymbol}${stats.revenue.toLocaleString('en-IN')}` },
            { l: 'Cancelled', v: stats.cancelled },
          ].map((s, i) => (
            <React.Fragment key={i}>
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-[#94a399]">{s.l}</span>
                <span className="text-[12px] font-bold text-[#111814]">{s.v}</span>
              </div>
              {i < 3 && <span className="h-4 w-px bg-[#dee2e6]" />}
            </React.Fragment>
          ))}
          <div className="flex-1" />
          <button
            className="flex items-center gap-1 px-2 py-1 text-[10px] text-[#47554d] rounded transition-colors opacity-40 cursor-not-allowed"
        ...
        <div className="flex items-center h-[44px] px-3 bg-white border-b border-[#dee2e6] shrink-0 gap-2">
          {/* Date presets */}
          <div className="flex items-center gap-0.5 bg-[#f1f5f2] rounded-lg p-0.5">
            {DATE_PRESETS.map((p) => (
              <button
                key={p.key}
                ...
                className={cn(
                  ...
                  datePreset === p.key
                    ? 'bg-white text-[#0f7b47] shadow-sm'
                    : 'text-[#757575] hover:text-[#47554d]',
                )}
              >
          ...
          {datePreset === 'custom' && (
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="h-7 px-2 text-[10px] border border-[#dee2e6] rounded focus:outline-none focus:ring-1 focus:ring-[#0f7b47]"
              />
              <span className="text-[10px] text-[#94a399]">to</span>
              <input
                type="date"
                value={customEnd}
              ...
              <button
                onClick={fetchOrders}
                className="h-7 px-2.5 text-[10px] font-medium bg-[#0f7b47] text-white rounded hover:bg-[#056638]"
              >
                Apply
              </button>
            </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="h-7 w-52 pl-7 pr-2 text-[10px] bg-[#f8faf9] border border-[#dee2e6] rounded focus:outline-none focus:ring-1 focus:ring-[#0f7b47]"
            />
            {searchQuery && (
              <button onClick={() => setSearchQuery('')} className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[#94a399] hover:text-[#111814]">
                <X className="h-3 w-3" />
              </button>
            )}
          </div>
      ...
      {viewOrder && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setViewOrder(null); }}>
          ...
          <div className="relative w-full max-w-[440px] bg-white rounded-xl shadow-2xl border border-[#dee2e6] overflow-hidden max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#dee2e6] bg-[#f8faf9] shrink-0">
              <h3 className="text-[13px] font-bold text-[#111814]">{viewOrder.orderNumber}</h3>
              <button onClick={() => setViewOrder(null)} className="p-1 rounded text-[#94a399] hover:text-[#111814]"><X className="h-4 w-4" /></button>
            </div>
            <div className="p-4 space-y-2 text-[11px] overflow-auto">
              {[
                ...
                ['Total', `${currSymbol}${viewOrder.total.toLocaleString('en-IN')}`],
                ...
                ...(viewOrder.walletAmountUsed > 0 ? [
                  ['Wallet Paid', `${currSymbol}${viewOrder.walletAmountUsed.toLocaleString('en-IN')}`],
                  ['Cash/Card Paid', `${currSymbol}${(viewOrder.total - viewOrder.walletAmountUsed).toLocaleString('en-IN')}`],
                ] : []),
                ...
                <div key={l as string} className="flex justify-between py-1.5 border-b border-[#f0f0f0]">
              ...
              {viewOrder._raw.cartRows && viewOrder._raw.cartRows.length > 0 && (
                <div className="pt-2">
                  ...
                  {viewOrder._raw.cartRows.map((item, idx) => (
                    <div key={idx} className="flex justify-between text-[10px] py-1 border-b border-[#f0f0f0]">
                      <span>{item.qty}× {item.itemName}</span>
                      <span className="tabular-nums font-medium">{currSymbol}{item.itemPrice.toLocaleString('en-IN')}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex gap-2 px-4 pb-4 pt-1 shrink-0">
              <button onClick={() => { handlePrint58mm(viewOrder); setViewOrder(null); }} className="flex-1 h-8 text-[10px] font-medium border border-[#dee2e6] rounded flex items-center justify-center gap-1 hover:bg-[#f1f5f2]">
      ...
      {editOrder && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setEditOrder(null); }}>
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
          <div className="relative w-full max-w-[380px] bg-white rounded-xl shadow-2xl border border-[#dee2e6] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#dee2e6] bg-[#f8faf9]">
              <h3 className="text-[13px] font-bold text-[#111814]">Edit {editOrder.orderNumber}</h3>
              <button onClick={() => setEditOrder(null)} className="p-1 rounded text-[#94a399] hover:text-[#111814]"><X className="h-4 w-4" /></button>
            </div>
            <div className="p-4 space-y-3">
              {/* Status */}
              <div>
                <label className="block text-[10px] font-semibold text-[#47554d] mb-1">Order Status</label>
                <select
                  value={editStatus}
              ...
              {/* Notes */}
              <div>
                <label className="block text-[10px] font-semibold text-[#47554d] mb-1">Notes</label>
                <textarea
                  value={editNotes}
                  onChange={(e) => setEditNotes(e.target.value)}
                  rows={2}
                  className="w-full px-3 py-2 text-[11px] border border-[#dee2e6] rounded-lg focus:outline-none focus:ring-1 focus:ring-[#0f7b47] resize-none"
                  placeholder="Order notes..."
                />
              </div>
              {/* Info */}
              <div className="text-[10px] text-[#94a399] bg-[#f8faf9] rounded-lg px-3 py-2">
                {editOrder.customer} · {editOrder.items} items · {currSymbol}{editOrder.total.toLocaleString('en-IN')} · {editOrder.paymentStatus}
              </div>
              {/* Actions */}
              <div className="flex gap-2 pt-1">
                <button onClick={() => setEditOrder(null)} className="flex-1 h-[34px] text-[11px] font-medium border border-[#dee2e6] text-[#47554d] rounded-lg hover:bg-[#f1f5f2]">
                  Cancel
                </button>
                <button onClick={handleSaveEdit} className="flex-1 h-[34px] text-[11px] font-semibold bg-[#0f7b47] text-white rounded-lg hover:bg-[#056638]">
                  Save Changes
                </button>
              </div>
            </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setDeleteConfirm(null); }}>
          ...
          <div className="relative w-full max-w-[320px] bg-white rounded-xl shadow-2xl border border-[#dee2e6] p-6 text-center animate-slide-up">
            ...
            <h3 className="text-[14px] font-bold text-[#111814] mb-1">Delete not available</h3>
            <p className="text-[11px] text-content-secondary mb-4">Order deletion from the POS is not yet implemented.</p>
            <div className="flex gap-2">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 h-[34px] text-[11px] font-medium border border-[#dee2e6] text-[#47554d] rounded-lg hover:bg-[#f1f5f2]">Close</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showReport && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) setShowReport(false); }}>
          <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" />
          <div className="relative w-[300px] bg-white rounded-xl shadow-2xl border border-[#dee2e6] overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 border-b border-[#dee2e6] bg-[#f8faf9]">
              <h3 className="text-[13px] font-bold">{statsLabel} Report — 58mm</h3>
              <button onClick={() => setShowReport(false)} className="p-1 rounded text-[#94a399] hover:text-[#111814]"><X className="h-4 w-4" /></button>
            </div>
            <div className="p-4">
              <div className="bg-white border-2 border-dashed border-[#dee2e6] rounded-lg p-4 font-mono text-[10px] leading-relaxed">
                ...
                <div className="border-t border-dashed border-[#dee2e6] my-2" />
                {[
                  ['Total Orders', stats.total],
                  ['Active Orders', stats.active],
                  ['Revenue', `${currSymbol}${stats.revenue.toLocaleString('en-IN')}`],
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={() => setShowReport(false)} className="flex-1 h-8 text-[10px] font-medium border border-[#dee2e6] rounded hover:bg-[#f1f5f2]">Close</button>
                <button onClick={todayReport58mm} className="flex-1 h-8 text-[10px] font-semibold bg-[#0f7b47] text-white rounded hover:bg-[#056638] flex items-center justify-center gap-1"><Printer className="h-3 w-3" />Print 58mm</button>
              </div>
            </div>
          </div>
        </div>
